...
class MedicalPatients(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for partner in self:
            name = partner.with_context(lang=ctx.get('lang'))._get_complete_name()
            ...
            # Show database ID
            if ctx.get('partner_show_db_id'):
                name = f"{name} ({partner.id})"

            # Inline address
            if ctx.get('address_inline'):
                name = ", ".join(
                    part.strip()
            ...
            # Show email
            if ctx.get('show_email') and partner.email:
                name = f"{name} <{partner.email}>"

            # Show VAT
            if ctx.get('show_vat') and partner.vat:
    ...
    def action_view_appointments(self):
        ...
        action = self.env.ref('adm_physiotherapy.medical_appointment_menu_action').read()[0]
